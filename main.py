...
def load_image(name, colorkey=None):
    fullname = os.path.join('../pythonProject10/data', name)
...
k = pygame.mixer.Sound('../pythonProject10/data/norm_music.mp3')
k.play(-1)

sound = pygame.mixer.Sound('../pythonProject10/data/shot.mp3')
...
bobrs_list = [load_image("bobr_alive.jpg"), load_image("bobr_hunter.png"), load_image("bobr_murder.png"),
              load_image("bobr_boom.png"), load_image("bobr_jeday.png"), load_image("bobr_soldier.png"),
...
MANUAL_CURSOR = pygame.image.load('data/3.png').convert_alpha()
    ...
    for event in pygame.event.get():
        if event.type == pygame.USEREVENT and not isBoss:
            if hole_list[bobr_hole]:
                score -= 1
                score = max(score, 0)
                pygame.mixer.music.load('../pythonProject10/data/BOSS_MUSIC.mp3')
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            if mouse_pos[0] in list(range(30, 120)) and mouse_pos[1] in list(range(130, 220)) and hole_list[0] and not isBoss:
                ...
                score -= 1
            elif mouse_pos[0] in list(range(30, 420)) and mouse_pos[1] in list(range(120, 520)):
                score += 1
                if score == 20:
                    isBoss = False
                    pygame.mixer.music.load('../pythonProject10/data/norm_music.mp3')
            ...
            if score == 30:
                pygame.time.set_timer(pygame.USEREVENT, 0)
                isBoss = True
                pygame.mixer.music.load('../pythonProject109/data/BOSS_MUSIC.mp3')
                pygame.mixer.music.play(-1)
